...
import { Star, ArrowRight, Heart, Stethoscope, Scissors, Syringe, ShieldCheck, HeartHandshake, Award, Clock } from 'lucide-react';
...
export default function HomePage() {
  ...
  return (
    <div>
      {/* Hero Section */}
      <section
        className="relative overflow-hidden from-slate-900 via-blue-950 to-slate-900 py-24 px-4"
        style={{
      ...
      <section className="bg-blue-600 py-8">
        <div className="max-w-6xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8 text-center text-white">
          <div>
            ...
            <p className="text-sm text-blue-100 mt-1">Pasien</p>
          </div>
          <div>
            <p className="text-3xl font-bold">10+</p>
            <p className="text-sm text-blue-100 mt-1">Dokter</p>
          </div>
          <div>
            <p className="text-3xl font-bold">15+</p>
            <p className="text-sm text-blue-100 mt-1">Layanan</p>
          </div>
          <div>
            <p className="text-3xl font-bold">5★</p>
            <p className="text-sm text-blue-100 mt-1">Rating</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-20 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-slate-900 dark:text-slate-100">Kenapa Kami</h2>
            <p className="mt-3 text-slate-500 dark:text-slate-400">Alasan memilih PetCare Suite untuk hewan kesayangan Anda</p>
          </div>
          <div className="grid gap-8 md:grid-cols-3">
            <div className="text-center">
              ...
              <h3 className="mt-4 text-lg font-semibold text-slate-900 dark:text-slate-100">Penuh Kasih</h3>
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Kami merawat setiap hewan dengan penuh cinta dan perhatian seperti keluarga sendiri.</p>
            </div>
            <div className="text-center">
              <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100 text-emerald-600">
                <Award className="h-8 w-8" />
              </div>
              <h3 className="mt-4 text-lg font-semibold text-slate-900 dark:text-slate-100">Berpengalaman</h3>
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Tim dokter hewan profesional dengan pengalaman bertahun-tahun di bidangnya.</p>
            </div>
            <div className="text-center">
              <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100 text-emerald-600">
                <Clock className="h-8 w-8" />
              </div>
              <h3 className="mt-4 text-lg font-semibold text-slate-900 dark:text-slate-100">Siaga 7 Hari</h3>
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Klinik buka setiap hari untuk memastikan hewan Anda selalu mendapatkan perawatan terbaik.</p>
            </div>
          </div>
        </div>
      </section>
              ...
              {doctorsQuery.isLoading
                ...
                : doctorsQuery.data.map((doc: any) => (
                    <Card key={doc.id} className="p-6 text-center transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
                      ...
                      <Badge variant="blue" className="mt-2">{doc.specialization}</Badge>
                    </Card>
                  ))}
            </div>
          </div>
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {testimonialsQuery.isLoading
                ...
                : testimonialsQuery.data.map((t: any) => (
                    <Card key={t.id} className="p-6 transition-all duration-200 hover:shadow-lg">
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {articlesQuery.isLoading
                ...
                : articlesQuery.data.map((a: any) => (
                    <Card key={a.id} className="overflow-hidden transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
                      {a.cover_url ? (
      ...
      {/* CTA Section */}
      <section className="from-blue-700 to-blue-900 py-20 px-4" style={{ background: 'linear-gradient(to bottom, #1d4ed8, #1e3a8a)' }}>
